...
const addEvent = async () => {
        if (editingEventId.value) {
            const res = await axios.put(`http://127.0.0.1:8000/api/events/${editingEventId.value}`,
            newEvent.value)
            
            const index = events.value.findIndex(e => e.id === editingEventId.value)
            events.value[index] = res.data
        } else {
        const res = await axios.post(`http://127.0.0.1:8000/api/events`, newEvent.value)
...
const openQuestions = async(eventId) => {
    ...
    const res = await axios.get(`http://127.0.0.1:8000/api/events/${eventId}/questions`)
...
const addQuestion = async() => {
    ...
    if(editingQuestionId.value) {
        ...
        res = await axios.post(
            `http://127.0.0.1:8000/api/events/${currentEventId.value}/questions`, {
                question: newQuestion.value
            }
        )
    }
                ...
                <tr 
                    ...
                    <td style="padding: 15px;">
                        <div v-if="event.questions && event.questions.length">
                            ...
                            <button @click="openQuestions(event.id)">
                                Edit Questions
                            </button>
                        </div>
